...
INCREMENT_BASES = [INCREMENT_PER_PERIOD * w for w in WEIGHTS]
...
def run_backtest(price_data, bt_start, bt_end):
    ...
    for date in sample_dates:
        ...
        # Phase 1: compute planned operations for all targets
        plans = []
        ...
        # Phase 2: if paused, cap total spending by reserve pool budget
        if paused:
...
# ═══════════════════════════════════════════════════════════
# Download data & generate both files
# ═══════════════════════════════════════════════════════════
if __name__ == "__main__":
    ...
    for tname, ticker in YFINANCE_TICKERS.items():
        df_etf = yf.download(ticker, start="2005-01-01", end="2026-04-23", progress=False)
        ...
        df_idx = yf.download(idx_ticker, start="2005-01-01", end="2026-04-23", progress=False)
    ...
    BASE_DIR = "/Users/youxingzhi/ayou/financial_strategy"
    OUTPUT_PATH = f"{BASE_DIR}/定投计划.xlsx"
    ...
    with open(f"{BASE_DIR}/cache/price_data.pkl", "wb") as f:
        pickle.dump(price_data, f)
    with open(f"{BASE_DIR}/cache/backtest_results.pkl", "wb") as f:
