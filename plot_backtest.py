...
import sys
sys.path.insert(0, '.')
from generate_plan import TargetState
import matplotlib
matplotlib.use('Agg')
...
with open('cache/price_data.pkl', 'rb') as f:
    price_data = pickle.load(f)

with open('cache/backtest_results.pkl', 'rb') as f:
    all_results = pickle.load(f)

import openpyxl
wb = openpyxl.load_workbook('定投计划.xlsx', data_only=True)
...
# ── Chart 1: Overall returns (all 9 periods, grouped) ──
...
ann_rates = []
group_boundaries = []
color_map = {'2年': '#457B9D', '5年': '#E76F51', '10': '#264653', '20': '#7B2D8E'}
...
for group_name, group in GROUPS:
    ...
    for p in group:
        ...
        key = p[:2]
        x_colors.append(color_map.get(key, '#457B9D'))
        pos += 1
...
# ── Chart 2: Per-ETF returns (subplots per group) ──
n_groups = len(GROUPS)
...
for ax_idx, (group_name, group) in enumerate(GROUPS):
    ...
    for i, etf in enumerate(ETF_NAMES):
        ...
        offset = (i - 1.5) * width
        bars = ax.bar(x + offset, etf_rates, width, label=etf, color=COLORS[etf],
...
# ── Chart 3: Capital breakdown (all 9 periods) ──
...
# ── Chart 4: Annualized returns comparison ──
...
bars = ax.bar(x_pos, ann_rates_all, color=[color_map.get(p[:2], '#457B9D') for p in ALL_PERIODS],
...
# ── Chart 5: Portfolio timelines (5Y and 10Y windows) ──
...
for period_key in timeline_periods:
    ...
    for i in range(n_periods):
        ...
        paused = global_cum >= gp.PAUSE_TOTAL
        if paused:
            reserve_pool += gp.INCREMENT_PER_PERIOD
            increment_cum += gp.INCREMENT_PER_PERIOD
            reserve_pool *= 1 + gp.RESERVE_INTEREST_ANNUAL * 14 / 365
    ...
    for label in ax.get_yticklabels():
        label.set_fontproperties(font_prop)
    ax.yaxis.set_major_formatter(FuncFormatter(lambda x, _: f'{x/10000:.0f}万'))
    fig.tight_layout()
