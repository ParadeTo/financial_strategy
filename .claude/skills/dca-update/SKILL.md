...
- **目标市值** = 基准金额 × 期数（增量阶段按 2000/6000 比例缓增）
- **每期操作** = 目标市值 − 持仓市值（正=买入缺口，负=卖出超额）
- **减半清仓**：120日均线偏离度 ≥ +35% 且持仓 > 目标市值×50%，卖至目标50%，不进冷却
...
for sname, ticker in TICKERS.items():
    ...
    prices[sname] = {"price": price, "ma120": ma120, "date": str(close.index[-1].date())}
...
HEADER = f"{'标的':<10} {'价格':>10} {'MA120':>10} {'偏离度':>8} {'持仓份额':>10} {'持仓市值':>10} {'目标市值':>10} {'操作建议':<10} {'建议金额':>12} {'建议份额':>10}"
print(HEADER)
print("─" * len(HEADER))
...
for sname in ETF_SHEETS:
    ...
    price = p["price"]
    ma120 = p["ma120"]
    dev   = (price - ma120) / ma120
    sh    = state[sname]["shares"]
    hold  = sh * price

    # 增量阶段目标市值按比例缓增（此处用上期期数估算）
    effective_period = next_period  # 简化：冷却期数偏移由用户自行查阅
    tgt = base * effective_period
    if paused:
        # 增量阶段：每期只增长 base × (2000/6000)
        prev_tgt = base * state[sname]["last_period"]
...
若任何 ticker 获取失败，向用户请求手动输入该标的的「当前价格」和「120日均线」，然后重新计算。
...
for sname, actual in actuals.items():
    ...
    ws.cell(row=write_row, column=5).value  = prices[sname]["ma120"]       # E 120日均线
